...
ONTAP 9.8 added support for FlexGroup datastores in vSphere, along with the ONTAP tools for VMware vSphere 9.8 release. FlexGroup volumes simplify the creation of large datastores and automatically create the necessary distributed constituent volumes to get maximum performance from an ONTAP system. You can learn more about FlexGroup volumes https://docs.netapp.com/us-en/ontap-technical-reports/nas-containers.html[here].
...
You may find that if you use VAAI cloning, but do not clone enough to keep the cache warm, your clones may be no faster than a host-based copy. If that is the case, you might have to tune the cache timeout to better suit your needs.
...
* FlexGroup vSphere datastore support has been tested up to 1500 VMs with the 9.8 release, but that is not a technical limit.
