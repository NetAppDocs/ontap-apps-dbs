...
searchtitle: Thin provisioning with Oracle databases
---
= Thin provisioning
:hardbreaks:
:nofooter:
...
The efficiency of thin provisioning of active LUNs in a file system environment can be lost over time as data is deleted. Unless that deleted data is either overwritten with zeros (see also link:oracle-storage-san-config-asm 
