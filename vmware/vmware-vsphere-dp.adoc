...
Additionally, the SnapCenter Plug-In for VMware can integrate with https://docs.netapp.com/us-en/bluexp-backup-recovery/concept-protect-vm-data.html[BlueXP for backup and recovery of virtual machines^], enabling effective 3-2-1 backup solutions for most ONTAP systems. Note that some fees may apply if using BlueXP backup and recovery for virtual machines with premium services, such as object stores for additional backup storage. This section outlines the various options available for protecting your VMs and datastores.
