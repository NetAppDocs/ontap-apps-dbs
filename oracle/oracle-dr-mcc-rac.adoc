...
searchtitle: Oracle Extended RAC on MetroCluster
---
= Oracle Extended RAC
...
[NOTE]
If boot LUNs, including virtualized boot disks, are used with Oracle RAC, the `misscount` parameter might need to be changed. For more information about RAC timeout parameters, see link:oracle-app-config-rac.html[Oracle RAC with ONTAP].
