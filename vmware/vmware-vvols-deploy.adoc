...
. Create the Storage Virtual Machine (SVM) and its protocol configuration. You will select NVMe/FC, NFSv3, NFSv4.1, iSCSI, FCP, or a mix of those options. NVMe/TCP may also be used for traditional VMFS datastores. You may use either ONTAP System Manager wizards, or the cluster shell command line.
