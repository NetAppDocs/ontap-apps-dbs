...
ONTAP provides enterprise-class block storage for VMware vSphere using traditional iSCSI and Fibre Channel Protocol(FCP, or FC for short), and the incredibly performant and CPU efficient next-generation block protocol NVMe over Fabrics (NVMe-oF); with both NVMe/FC and NVMe/TCP support.

For detailed best practices for implementing block protocols for VM storage with vSphere and ONTAP refer to link:vmware-vsphere-datastores-nfs.html[Datastores and Protocols - SAN]
