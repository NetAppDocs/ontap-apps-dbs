...
keywords: MS-SQL,mssql,SQL Server, volumes, LUN, SnapCenter, log directory
summary: Microsoft SQL Server  storage consideration on ONTAP
---
= Microsoft SQL Server storage considerations on ONTAP
:imagesdir: ../media/

[.lead]
The combination of NetApp ONTAP storage solutions and Microsoft SQL Server enables the creation of enterprise-level database storage designs that can meet today's most demanding application requirements. 
