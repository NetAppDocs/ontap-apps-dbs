...
* For iSCSI or NVMe/TCP networks, use multiple VMkernel network interfaces on different unrouted network subnets with NIC teaming when multiple virtual switches are present. You can also use multiple physical NICs connected to multiple physical switches to provide HA and increased throughput. In ONTAP, configure either a single-mode interface group for failover with two or more links that are connected to two or more switches, or use LACP or other link-aggregation technology with multimode interface groups to provide HA and the benefits of link aggregation. Keep in mind that NVMe/TCP must be enabled on a per-VMkernel port and per physical interface, and you must align them such that NVMe/TCP VMkernel ports have access to physical NICs with NVMe/TCP enabled. The following figure illustrates multipath connectivity with IP-based SAN protocols. 
...
* For more on using NVMe/FC with vSphere 7.0, see this https://docs.netapp.com/us-en/ontap-sanhost/nvme_esxi_7.html[ONTAP NVMe/FC Host Configuration guide^] and https://www.netapp.com/pdf.html?item=/media/10681-tr4684.pdf[TR-4684^].The following figure depicts multipath connectivity from a vSphere host to an ONTAP LUN.
