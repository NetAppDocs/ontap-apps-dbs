...
The RTO can usually be measured in seconds. A an application is shut down, the volume/LUN/namespaces are restored, and the application is restarted. 

The simplest approach is to place all the files or LUNs in a single AFF volume or ASAr2 LUN/namespace. Where a dataset must span multiple storage resources, a consistency group is required. This can be configured using System Manager or RESTful API calls, plus SnapCenter is capable of creating a simple consistency group snapshot.
...
When a database must span AFF volumes or ASAr2 LUNs/namespaces, a consistency group snapshot (cg-snapshot) is required. As mentioned above, this can be configured using System Manager or RESTful API calls, plus SnapCenter is capable of creating a simple consistency group snapshot.

There is also one additional consideration on the use of multi-resource, consistent snapshots for disaster recovery purposes. When performing an update of multiple snapmirror relationships, it is possible that a disaster could occur while a transfer is still in progress. The result would be a set of volumes, LUNs or namespaces that are not consistent with one another. If this happened, some of the volumes must be restored to an earlier snapshot state to deliver a database image that is crash-consistent and ready for use.
