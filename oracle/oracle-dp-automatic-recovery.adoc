...
* A synchronized set of archive logs, controlfiles, and redo logs. This can be done by placing those files in a single volume and creating a snapshot or by using CG snapshots across multiple volmes. The logs must contain all records created from the time of the datafile snapshot through the last change recorded in the controlfiles.
