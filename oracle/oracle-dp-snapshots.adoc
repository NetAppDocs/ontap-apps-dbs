...
searchtitle: Oracle databases and snapshot-based backups
---
...
* Furthermore, multiple AFF volumes or ASAr2 LUNs/namespaces can be easily bonded together as a consistency group, with data protection policies applied as a single unit.

ONTAP snapshots also scale better than competing technology. Customers can store 5, 50, or 500 snapshots without affecting performance. The maximum number of snapshots currently allowed in an AFF volume or ASAr2 LUN/namespace is 1024. If additional snapshot retention is required, there are options to cascade snapshots.
