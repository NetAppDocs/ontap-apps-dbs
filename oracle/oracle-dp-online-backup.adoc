...
ASAr2 does not have the volume-level abstraction. Instead it uses consistency groups. In many cases, a single LUN or NVMe namespace might meet the management and performance requirements for a database. If multiple LUNs or namespaces is required, additional resources can be added and bound together as a consistency group. 
...
A complication arises in situations such as the use of an ASM disk group that is distributed across AFF volumes or ASAr2 LUNs/namespaces. In these cases, a consistency group must be defined to make sure that the ASM metadata is consistent across all constituent resources.
