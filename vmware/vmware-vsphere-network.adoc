...
* NetApp only recommends disabling network flow control on the cluster-interconnect ports within an ONTAP cluster. NetApp makes no other recommendations for best practices for the remaining network ports used for data traffic. You should enable or disable it as necessary. See https://www.netapp.com/pdf.html?item=/media/16885-tr-4182pdf.pdf[TR-4182^] for more background on flow control.
