...
searchtitle: Microsoft SQL Server tempdb placement on ONTAP
---
= Tempdb files
:hardbreaks:
:icons: font
...
[.lead]
The Tempdb database can be heavily utilized. In addition to optimal placement of user database files on ONTAP, tempdb  datafiles placement is also critical to reduce allocation contention. Tempdb should be placed on separate disk and not shared with user datafiles.
