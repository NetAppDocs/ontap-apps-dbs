...
summary: LVM stripe configuration for Oracle databases with ASA r2
searchtitle: LVM striping with Oracle databases on ASA r2
---
= LVM striping
:hardbreaks:
:nofooter:
...
* Stripe size ≥ database block size.
* Stripe size * number of LUNs ≈ 1MB for optimal parallelism.
